    ...
    ILogger<WhatsAppCredentialNotifier> logger)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(
                "Envoi WhatsApp des identifiants non configuré pour {PhoneNumber}.",
                phoneNumber);
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var body = await response.Content.ReadAsStringAsync(
                    cancellationToken);
                logger.LogWarning(
                    "Échec de l'envoi WhatsApp des identifiants ({StatusCode}) : {ResponseBody}",
                    response.StatusCode,
                    body);
                return "Failed";
            }
        ...
        catch (Exception exception)
        {
            logger.LogError(
                exception,
                "Échec de l'envoi WhatsApp des identifiants à {PhoneNumber}.",
                phoneNumber);
            return "Failed";
        }
    }
