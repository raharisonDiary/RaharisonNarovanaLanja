    ...
    : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var childrenByParent = areas
            .Where(value => value.ParentId.HasValue)
            .ToLookup(value => value.ParentId!.Value);
        var descendants = GetDescendants(
            childrenByParent,
            selectedAreaId);

        var dwellingQuery = dbContext.Dwellings
            .AsNoTracking()
            .Where(value =>
                value.CampaignId == campaignId &&
                value.RecordStatus == DwellingRecordStatus.Validated);

        // When the selected territory covers the entire hierarchy, avoid
        // sending more than twenty thousand area identifiers to PostgreSQL.
        if (descendants.Count < areas.Count)
        {
            dwellingQuery = dwellingQuery.Where(value =>
                descendants.Contains(value.EnumerationAreaId));
        }

        var dwellingRows = await dwellingQuery
    ...
    private static HashSet<Guid> GetDescendants(
        ILookup<Guid, AdministrativeArea> childrenByParent,
        Guid rootId)
    {
