    ...
    : IPersonService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await EnsureNoPotentialDuplicateAsync(
            household.CampaignId,
            household.Id,
            model.FirstName,
            model.LastName,
        ...
        var person = await personRepository.AddWithGeneratedNumberAsync(
            household.Id,
            personNumber =>
            {
    ...
    private async Task EnsureNoPotentialDuplicateAsync(
        Guid campaignId,
        Guid householdId,
        string firstName,
        string lastName,
    ...
    {
        var duplicate =
            await personRepository.FindPotentialDuplicateAsync(
                campaignId,
                householdId,
                firstName,
                lastName,
        ...
        throw new ConflictException(
            $"Doublon probable detecte : {duplicate.FullName} " +
            $"existe deja dans cette campagne " +
            $"(personne n° {duplicate.PersonNumber}).");
    }
