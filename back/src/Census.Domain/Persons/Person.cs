...
public sealed class Person : EntityBase
{
        ...
        DateTimeOffset createdAtUtc)
    {
        ...
        ValidateChildrenCount(childrenCount);
    ...
    public BirthDatePrecision BirthDatePrecision { get; private set; }
    ...
    public DateOnly? NationalIdIssueDate { get; private set; }
    public string? NationalIdIssuePlace { get; private set; }
    public string? PhotoDataUrl { get; private set; }
    public string? Notes { get; private set; }
    public PersonRecordStatus RecordStatus { get; private set; }
        ...
        string? notes)
    {
        ...
        AgeYears = ageYears;
        BirthDatePrecision = birthDatePrecision;
        BirthPlace = NormalizeOptional(birthPlace, 150);
        RelationshipToHead = relationshipToHead;
        MaritalStatus = maritalStatus;
        ChildrenCount = maritalStatus is
            MaritalStatus.Married or MaritalStatus.Widowed
            ? childrenCount
            : null;
        ...
        NationalId = NormalizeOptional(nationalId, 100)
            ?.ToUpperInvariant();

        NationalIdIssueDate = NationalId is null
            ? null
            : nationalIdIssueDate;

        NationalIdIssuePlace = NationalId is null
            ? null
            : NormalizeOptional(nationalIdIssuePlace, 150);

        PhotoDataUrl = NormalizePhotoDataUrl(photoDataUrl);
    ...
    private static void ValidateChildrenCount(int? childrenCount)
    {
        if (childrenCount is < 0 or > 50)
        {
            throw new ArgumentOutOfRangeException(
                nameof(childrenCount),
                "Le nombre d’enfants doit être compris entre 0 et 50.");
        }
    }
        ...
        BirthDatePrecision birthDatePrecision)
    {
        if (!Enum.IsDefined(sex) ||
            ...
            !Enum.IsDefined(birthDatePrecision))
        {
    ...
    private static string? NormalizePhotoDataUrl(string? value)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            return null;
        ...
        {
            throw new ArgumentException(
                "La photo doit être une image encodée valide de moins de 5 Mo.");
        }
