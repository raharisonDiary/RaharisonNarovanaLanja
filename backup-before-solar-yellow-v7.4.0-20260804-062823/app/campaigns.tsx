...
import { nextCampaignStatuses } from '../src/utils/lifecycle'
...
const makeCode = (name: string) =>
  `${name
    ...
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-zA-Z0-9]/g, '')
    .slice(0, 12) || 'CENSUS'}-${new Date().getFullYear()}`.toUpperCase()
...
const children = (
  ...
  (area) => area.parentId === parentId && area.type === type,
)
...
export default function CampaignsScreen() {
  ...
  const create = async () => {
    ...
    setError('')
    try {
      const created = await mobileApi.createCampaign({
        code: code.trim() || makeCode(name),
        name: name.trim(),
      ...
      if (launchMode !== 'Draft') {
        await mobileApi.changeCampaignStatus(created.id, launchMode)
      }
      setOpen(false)
      reset()
      await load()
  ...
  const changeStatus = async (
    ...
    if (!nextCampaignStatuses(campaign.status).includes(status)) return

    setChangingId(campaign.id)
    setError('')
    try {
      await mobileApi.changeCampaignStatus(campaign.id, status)
      await load()
    } catch (exception) {
      setError(messageFromError(exception))
    } finally {
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.title}>
            {tr(language, 'Campagnes', 'Fanisana', 'Campaigns')}
          </Text>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.list}>
          {filtered.map((item) => {
            const nextStatuses = nextCampaignStatuses(item.status)
            return (
              <View key={item.id} style={styles.card}>
                <View style={styles.cardTop}>
                  <View style={styles.icon}>
                    <CalendarClock color={colors.primary} size={21} />
                  </View>
                  <View style={{ flex: 1 }}>
                ...
                {canManageCampaigns(user?.role) && nextStatuses.length > 0 ? (
                  <ScrollView
                    horizontal
      ...
      >
        ...
        >
          <View style={styles.header}>
            ...
            <Text style={styles.title}>
              {tr(language, 'Nouvelle campagne', 'Fanisana vaovao', 'New campaign')}
            </Text>
          </View>
          <FormField
          ...
          />
          <FormField
            label={tr(language, 'Code (automatique si vide)', 'Kaody (mandeha ho azy)', 'Code (automatic if empty)')}
            value={code}
          ...
          />
          <FormField
            label={tr(language, 'Date de fin (AAAA-MM-JJ)', 'Daty fiafarana', 'End date (YYYY-MM-DD)')}
            value={endDate}
          ...
          />
          <SelectField
            label={tr(language, 'Pays', 'Firenena', 'Country')}
            value={countryId}
            onChange={(value) => {
              setCountryId(value)
              setRegionId('')
            ...
            options={countries.map((area) => ({ label: area.name, value: area.id }))}
          />
          {scopeType !== 'Country' ? (
            <SelectField
              ...
              options={regions.map((area) => ({ label: area.name, value: area.id }))}
            />
          ) : null}
          {['District', 'Commune'].includes(scopeType) ? (
            <SelectField
          ...
          <SelectField
            label={tr(language, 'Mode de lancement', 'Fomba fanombohana', 'Launch mode')}
            value={launchMode}
          ...
          />
          <FormField
            label={tr(language, 'Description', 'Fanazavana', 'Description')}
            value={description}
...
const styles = StyleSheet.create({
  ...
  statusText: { fontSize: 9, color: colors.primary, fontWeight: '800' },
