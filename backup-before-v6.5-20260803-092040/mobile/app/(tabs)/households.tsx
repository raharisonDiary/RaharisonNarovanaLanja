...
export default function HouseholdsScreen() {
  ...
  const runServerAction = async (item: HouseholdDto, action: () => Promise<unknown>) => {
    setWorkingId(item.id); setError('')
    try { await action(); await load() }
    catch (exception) { setError(messageFromError(exception)) }
    finally { setWorkingId('') }
  }
  const removeServer = (item: HouseholdDto) => Alert.alert(tr(language, 'Supprimer ce ménage ?', 'Hamafa ity tokantrano ity?', 'Delete this household?'), '', [{ text: t('cancel') }, { text: t('delete'), style: 'destructive', onPress: () => void runServerAction(item, () => mobileApi.deleteHousehold(item.id)) }])
  ...
  return <View style={styles.screen}>
    ...
    {error && !editing ? <Text style={[styles.error, { marginHorizontal: spacing.lg }]}>{error}</Text> : null}
    {loading ? <ActivityIndicator style={{ marginTop: 40 }} color={colors.primary} /> : <ScrollView contentContainerStyle={styles.list}>{rows.length === 0 ? <View style={styles.empty}><UsersRound color={colors.muted} size={38} /><Text style={styles.emptyText}>{t('noHousehold')}</Text></View> : rows.map((item) => {
      ...
      const campaignActive = server ? campaigns.find((campaign) => campaign.id === server.campaignId)?.status === 'Active' : false
      ...
      return <View key={item.key} style={styles.card}><View style={styles.cardHead}>{item.source === 'local' ? <WifiOff color={colors.warning} size={18} /> : <Cloud color={colors.success} size={18} />}<View style={{ flex: 1 }}><Text style={styles.code}>{item.referenceCode}</Text><Text style={styles.name}>{item.headFullName}</Text><Text style={styles.meta}>{item.source === 'local' && local ? `${local.payload.location.fokontanyName} · ${local.payload.persons.length} ${t('citizenCount')}` : item.phone || t('synced')}</Text></View><StatusPill value={item.source === 'local' ? t('pendingSync') : item.status} /></View>{local ? <View style={styles.actions}><Action icon={Eye} label={t('details')} onPress={() => router.push(`/households/${local.id}` as never)} /><Action icon={Pencil} label={t('edit')} onPress={() => router.push({ pathname: '/households/new', params: { editId: local.id } })} /><Action icon={Trash2} label={t('delete')} danger onPress={() => removeLocal(local)} /></View> : server && (editable || submittable || validatable || deletable) ? <View style={styles.actions}>{editable ? <Action icon={Pencil} label={t('edit')} disabled={workingId === server.id} onPress={() => beginEdit(server)} /> : null}{submittable ? <Action icon={Send} label={tr(language, 'Soumettre', 'Handefa', 'Submit')} disabled={workingId === server.id} onPress={() => void runServerAction(server, () => mobileApi.submitHousehold(server.id))} /> : null}{validatable ? <Action icon={CheckCircle2} label={tr(language, 'Valider', 'Hankatoa', 'Validate')} disabled={workingId === server.id} onPress={() => void runServerAction(server, () => mobileApi.validateHousehold(server.id))} /> : null}{deletable ? <Action icon={Trash2} label={t('delete')} danger disabled={workingId === server.id} onPress={() => removeServer(server)} /> : null}</View> : null}</View>
    })}</ScrollView>}
