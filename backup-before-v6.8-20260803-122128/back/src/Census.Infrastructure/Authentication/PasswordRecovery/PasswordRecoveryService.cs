    ...
    : IPasswordRecoveryService
{
    private const string GenericMessage =
        "Si un compte actif correspond à cette adresse, un code de vérification a été envoyé.";
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return new PasswordRecoveryRequestResult(GenericMessage, null, null);
        }
        ...
        var expiresAtUtc = now.AddMinutes(_options.OtpLifetimeMinutes);

        user.BeginPasswordRecovery(codeHash, salt, expiresAtUtc, now);
        await userRepository.SaveChangesAsync(cancellationToken);

        await notifier.SendCodeAsync(
            user.Email,
            user.FullName,
            code,
            expiresAtUtc,
        ...
        var developmentOtp = environment.IsDevelopment() &&
                             _options.ExposeOtpInDevelopment
            ? code
            : null;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (user.PasswordRecoveryAttemptCount >= _options.MaximumVerificationAttempts)
        {
        ...
        try
        {
            saltBytes = Convert.FromBase64String(user.PasswordRecoveryCodeSalt);
        }
        ...
        var expectedHash = Convert.FromBase64String(user.PasswordRecoveryCodeHash);
        var actualHash = Convert.FromBase64String(ComputeCodeHash(normalizedCode, saltBytes));
        ...
        var tokenExpiresAtUtc = now.AddMinutes(_options.ResetTokenLifetimeMinutes);

        user.VerifyPasswordRecovery(tokenHash, tokenExpiresAtUtc, now);
        await userRepository.SaveChangesAsync(cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var expectedHash = Convert.FromBase64String(user.PasswordRecoveryTokenHash);
        var actualHash = Convert.FromBase64String(ComputeTokenHash(resetToken.Trim()));
        ...
        var sessions = await sessionRepository.GetActiveByUserIdForUpdateAsync(
            user.Id,
            now,
            cancellationToken);
    ...
    private static string ComputeCodeHash(string code, byte[] salt)
    {
