...
$backupRoot = Join-Path $projectRoot "backup-before-aurora-sidebar-v7.1.1-$timestamp"
...
# Preflight: cette version de React Native expose StyleSheet.absoluteFill,
# pas StyleSheet.absoluteFillObject.
$sidebarPayload = Join-Path $payloadRoot 'src\components\CensusSidebar.tsx'
$sidebarSource = Get-Content -LiteralPath $sidebarPayload -Raw
if ($sidebarSource -match 'StyleSheet\.absoluteFillObject') {
    throw 'Le correctif contient encore StyleSheet.absoluteFillObject. Installation annulee.'
}

Write-Host 'Installation Aurora Sidebar V7.1.1...' -ForegroundColor Cyan
...
Write-Host ''
Write-Host 'Aurora Sidebar V7.1.1 est installe.' -ForegroundColor Green
