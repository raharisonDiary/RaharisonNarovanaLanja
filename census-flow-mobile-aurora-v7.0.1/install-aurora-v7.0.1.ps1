...
if ($package.name -ne 'census-mobile') {
    throw "Le dossier mobile detecte ne correspond pas a Census Flow."
}

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot = Join-Path $ProjectRoot "backup-before-aurora-v7.0.1-$timestamp"
...
Write-Host 'Installation du design mobile Aurora Civic V7.0.1...' -ForegroundColor Cyan
...
$backupMarker = Join-Path $ProjectRoot '.aurora-v7.0.1-last-backup.txt'
...
if (-not $SkipTypecheck) {
    ...
    try {
        ...
        Write-Host ''
        Write-Host 'Echec de la verification. Restauration automatique...' -ForegroundColor Red
...
Write-Host ''
Write-Host 'Aurora Civic V7.0.1 est installe.' -ForegroundColor Green
