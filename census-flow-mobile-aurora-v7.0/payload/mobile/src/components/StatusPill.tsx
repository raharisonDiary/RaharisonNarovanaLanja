...
export default function StatusPill({ value }: { value: string }) {
  ...
  const tone = positive
    ? { bg: colors.successSoft, fg: colors.successDark, Icon: CheckCircle2 }
    : negative
      ? { bg: colors.dangerSoft, fg: colors.dangerDark, Icon: CircleAlert }
      : pending
        ? { bg: colors.warningSoft, fg: colors.warningDark, Icon: Clock3 }
        : { bg: colors.primarySoft, fg: colors.primary, Icon: Clock3 }

  const ToneIcon = tone.Icon

  return (
    <View style={[styles.pill, { backgroundColor: tone.bg }]}>
      <ToneIcon color={tone.fg} size={11} />
