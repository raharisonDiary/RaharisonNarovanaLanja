...
export default function NewHouseholdScreen() {
  ...
  useEffect(() => {
    ...
    Animated.parallel([
      Animated.timing(contentOpacity, { toValue: 1, duration: 280, easing: Easing.out(Easing.cubic), useNativeDriver: Platform.OS !== 'web' }),
      Animated.timing(contentTranslate, { toValue: 0, duration: 280, easing: Easing.out(Easing.cubic), useNativeDriver: Platform.OS !== 'web' }),
    ]).start()
  ...
  const optionalLabel = (label: string) => `${label} (${t('optional')})`
  ...
  const addCitizen = () => {
    if (!citizen.photoUri || !citizen.firstName.trim() || !citizen.lastName.trim() || !citizen.birthPlace.trim() || !citizen.occupation?.trim()) { setError(t('requiredCitizenFields')); return }
    if (adult && !citizen.hasNoNationalId && !citizen.nationalId?.trim()) { setError(t('nationalId')); return }
    if (['Married', 'Widowed'].includes(citizen.maritalStatus) && (citizen.childrenCount === null || citizen.childrenCount === undefined)) { setError(t('childrenCount')); return }
    ...
    const normalizeDuplicate = (value?: string | null) => (value ?? '').trim().toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    const duplicate = persons.find((person) => {
      const sameNationalId = Boolean(normalized.nationalId?.trim()) && normalizeDuplicate(person.nationalId) === normalizeDuplicate(normalized.nationalId)
      if (sameNationalId) return true
      const sameIdentity = normalizeDuplicate(person.firstName) === normalizeDuplicate(normalized.firstName) && normalizeDuplicate(person.lastName) === normalizeDuplicate(normalized.lastName)
      if (!sameIdentity) return false
      if (normalized.birthDateMode === 'exact' && normalized.dateOfBirth) return person.dateOfBirth === normalized.dateOfBirth
      if (normalized.birthDateMode === 'year' && normalized.birthYear) return person.birthYear === normalized.birthYear
      return person.ageYears === normalized.ageYears && normalizeDuplicate(person.birthPlace) === normalizeDuplicate(normalized.birthPlace)
    })
    if (duplicate) {
      setError(tr(language, `Doublon probable : ${duplicate.firstName} ${duplicate.lastName} est déjà ajouté.`, `Mety efa misy io olona io : ${duplicate.firstName} ${duplicate.lastName} dia efa nampidirina.`, `Possible duplicate: ${duplicate.firstName} ${duplicate.lastName} has already been added.`))
      return
    }
  ...
  const canContinue = () => {
    ...
    if (step === 5) return citizen.birthDateMode === 'unknown' || (age !== null && age >= 0 && age <= 130)
    if (step === 6) return adult && !citizen.hasNoNationalId ? Boolean(citizen.nationalId?.trim()) : true
    if (step === 7) return ['Married', 'Widowed'].includes(citizen.maritalStatus) ? citizen.childrenCount !== null && citizen.childrenCount !== undefined : true
    if (step === 8) return Boolean(citizen.occupation?.trim())
    return true
  }

  return <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    <View style={styles.top}><Pressable style={styles.back} onPress={() => step > 0 ? setStep((value) => value - 1) : goBackOrReplace(router, '/(tabs)/households')}><ArrowLeft color={colors.text} size={20} /></Pressable><View style={{ flex: 1 }}><Text style={styles.title}>{editId ? t('editLocalHousehold') : t('newCensus')}</Text><Text style={styles.subtitle}>{t('step')} {step + 1} {t('outOf')} {totalSteps} · {t('localSave')}</Text></View><View style={styles.localBadge}><Save color={colors.accent} size={14} /><Text>{t('local')}</Text></View></View>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <Animated.View style={[styles.card, { opacity: contentOpacity, transform: [{ translateY: contentTranslate }] }]}>
        ...
        {step === 3 && <><Section icon={Camera} title={`1. ${t('citizenPhoto')}`} text={t('citizenPhotoText')} /><View style={[styles.photoSourceGrid, width < 430 && styles.photoSourceGridSmall]}><Pressable style={[styles.photoSourceButton, styles.photoSourceCamera]} onPress={() => void takePhoto(true)}><Camera color={colors.primary} size={22} /><View style={{ flex: 1 }}><Text style={styles.photoSourceTitle}>{citizen.photoUri ? t('retakePhoto') : t('takePhoto')}</Text><Text style={styles.photoSourceText}>{t('cameraSourceHint')}</Text></View></Pressable><Pressable style={styles.photoSourceButton} onPress={() => void importPhoto(true)}><ImagePlus color={colors.accent} size={22} /><View style={{ flex: 1 }}><Text style={styles.photoSourceTitle}>{t('importPhoto')}</Text><Text style={styles.photoSourceText}>{t('gallerySourceHint')}</Text></View></Pressable></View><View style={styles.citizenPhotoBox}>{citizen.photoUri ? <><Image source={{ uri: citizen.photoUri }} style={styles.citizenPhoto} /><Pressable style={styles.retakeFloating} onPress={() => void takePhoto(true)}><RefreshCw color="white" size={18} /></Pressable></> : <><View style={styles.photoEmptyIcon}><Camera color={colors.primary} size={36} /></View><Text style={styles.photoTitle}>{t('photoRequired')}</Text><Text style={styles.captureText}>{t('faceCentered')}</Text></>}</View></>}
        {step === 4 && <><Section icon={UserRound} title={`2. ${t('citizenIdentity')}`} text={t('identityText')} /><FormField label={requiredLabel(t('lastName'))} value={citizen.lastName} onChangeText={(value) => setCitizenValue('lastName', value)} autoCapitalize="characters" /><FormField label={requiredLabel(t('firstNames'))} value={citizen.firstName} onChangeText={(value) => setCitizenValue('firstName', value)} /><SelectField label={requiredLabel(t('sex'))} value={citizen.sex} onChange={(value) => setCitizenValue('sex', value as PersonSex)} options={[{ label: t('female'), value: 'Female' }, { label: t('male'), value: 'Male' }, { label: t('otherNotStated'), value: 'NotStated' }]} /><FormField label={requiredLabel(t('birthPlace'))} value={citizen.birthPlace} onChangeText={(value) => setCitizenValue('birthPlace', value)} /></>}
        {step === 5 && <><Section icon={UserRound} title={`3. ${t('citizenBirthAge')}`} text={t('birthAgeText')} /><Choice label={requiredLabel(t('knownInfo'))} value={citizen.birthDateMode} options={[['exact', t('exactDate')], ['year', t('yearOnly')], ['age', t('declaredAge')], ['unknown', t('unknown')]]} onChange={(value) => setCitizenValue('birthDateMode', value as CitizenDraft['birthDateMode'])} />{citizen.birthDateMode === 'exact' && <FormField label={requiredLabel(t('birthDate'))} value={citizen.dateOfBirth ?? ''} onChangeText={(value) => setCitizenValue('dateOfBirth', value)} placeholder="AAAA-MM-JJ" />}{citizen.birthDateMode === 'year' && <FormField label={requiredLabel(t('birthYear'))} value={citizen.birthYear ? String(citizen.birthYear) : ''} onChangeText={(value) => setCitizenValue('birthYear', value ? Number(value) : null)} keyboardType="number-pad" placeholder="1985" />}{citizen.birthDateMode === 'age' && <FormField label={requiredLabel(t('declaredAge'))} value={citizen.ageYears !== null && citizen.ageYears !== undefined ? String(citizen.ageYears) : ''} onChangeText={(value) => setCitizenValue('ageYears', value ? Number(value) : null)} keyboardType="number-pad" />}{citizen.birthDateMode === 'unknown' && <Choice label={requiredLabel(t('ageCategory'))} value={unknownAdult ? 'adult' : 'minor'} options={[['minor', t('under18')], ['adult', t('adult18')]]} onChange={(value) => setUnknownAdult(value === 'adult')} />}<View style={styles.ageResult}><Text>{t('automaticFilter')}</Text><Text style={styles.ageStrong}>{adult ? t('adultLabel') : t('minorLabel')}</Text></View></>}
        {step === 6 && <><Section icon={Check} title={`4. ${t('nationalIdCard')}`} text={adult ? t('cinOptional') : t('cinMinorText')} />{adult ? <><Choice label={requiredLabel(t('nationalIdCard'))} value={citizen.hasNoNationalId ? 'none' : 'has'} options={[['has', t('hasId')], ['none', t('noId')]]} onChange={(value) => setCitizenValue('hasNoNationalId', value === 'none')} />{!citizen.hasNoNationalId ? <><FormField label={requiredLabel(t('nationalId'))} value={citizen.nationalId ?? ''} onChangeText={(value) => setCitizenValue('nationalId', value)} keyboardType="number-pad" /><FormField label={optionalLabel(t('issueDate'))} value={citizen.nationalIdIssueDate ?? ''} onChangeText={(value) => setCitizenValue('nationalIdIssueDate', value)} placeholder="AAAA-MM-JJ" /><FormField label={optionalLabel(t('issuePlace'))} value={citizen.nationalIdIssuePlace ?? ''} onChangeText={(value) => setCitizenValue('nationalIdIssuePlace', value)} /></> : <View style={styles.informationCard}><Text style={styles.informationText}>{t('cinOptional')}</Text></View>}</> : <View style={styles.informationCard}><Text style={styles.informationText}>{t('cinMinorText')}</Text></View>}</>}
        {step === 7 && <><Section icon={UsersRound} title={`5. ${t('maritalStatus')}`} text={t('identityText')} /><SelectField label={requiredLabel(t('maritalStatus'))} value={citizen.maritalStatus} onChange={(value) => setCitizenValue('maritalStatus', value as MaritalStatus)} options={[{ label: t('single'), value: 'Single' }, { label: t('married'), value: 'Married' }, { label: t('widowed'), value: 'Widowed' }, { label: t('divorced'), value: 'Divorced' }, { label: t('separated'), value: 'Separated' }, { label: t('neutral'), value: 'NotStated' }]} />{['Married', 'Widowed'].includes(citizen.maritalStatus) ? <FormField label={requiredLabel(t('childrenCount'))} value={citizen.childrenCount !== null && citizen.childrenCount !== undefined ? String(citizen.childrenCount) : ''} onChangeText={(value) => setCitizenValue('childrenCount', value ? Number(value) : null)} keyboardType="number-pad" /> : null}<SelectField label={requiredLabel(t('relationship'))} value={citizen.relationshipToHead} onChange={(value) => setCitizenValue('relationshipToHead', value as RelationshipToHead)} options={[{ label: t('head'), value: 'Head' }, { label: t('spouse'), value: 'Spouse' }, { label: t('child'), value: 'Child' }, { label: t('parent'), value: 'Parent' }, { label: t('sibling'), value: 'Sibling' }, { label: t('otherRelative'), value: 'OtherRelative' }, { label: t('nonRelative'), value: 'NonRelative' }]} /></>}
        {step === 8 && <><Section icon={Check} title={`6. ${t('occupation')}`} text={t('citizensDraftText')} /><FormField label={requiredLabel(t('occupation'))} value={citizen.occupation ?? ''} onChangeText={(value) => setCitizenValue('occupation', value)} placeholder={t('occupationPlaceholder')} /><FormField label={optionalLabel(t('personalPhone'))} value={citizen.phoneNumber ?? ''} onChangeText={(value) => setCitizenValue('phoneNumber', value)} keyboardType="phone-pad" /><FormField label={optionalLabel(t('nationality'))} value={citizen.nationality ?? 'Malagasy'} onChangeText={(value) => setCitizenValue('nationality', value)} /><View style={styles.reviewCard}>{citizen.photoUri ? <Image source={{ uri: citizen.photoUri }} style={styles.reviewPhoto} /> : null}<View style={{ flex: 1 }}><Text style={styles.reviewName}>{citizen.firstName} {citizen.lastName}</Text><Text style={styles.reviewText}>{age ?? t('unknownAge')} {t('years')} · {adult ? t('adultLabel') : t('minorLabel')}</Text><Text style={styles.reviewText}>{citizen.occupation || '—'} · {citizen.nationality || '—'}</Text></View></View><PrimaryButton title={t('addThisCitizen')} icon={Check} disabled={!canContinue()} onPress={addCitizen} /></>}
        {error ? <Text style={styles.error}>{error}</Text> : null}
      </Animated.View>
    </ScrollView>
    {step !== 2 && <View style={styles.footer}>{step > 0 ? <Pressable style={styles.previous} onPress={() => setStep((value) => value - 1)}><ChevronLeft color={colors.textSoft} size={18} /><Text style={styles.previousText}>{t('previous')}</Text></Pressable> : <View />}{step < 8 && <View style={styles.next}><PrimaryButton title={t('continue')} icon={ChevronRight} disabled={!canContinue()} onPress={() => setStep((value) => value + 1)} /></View>}</View>}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 19, fontWeight: '900', color: colors.text, letterSpacing: -0.35 },
  subtitle: { fontSize: 10, color: colors.muted, marginTop: 2, lineHeight: 15 },
  localBadge: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  ...
  progress: { height: 5, backgroundColor: colors.surfaceMuted },
  progressFill: {
    height: '100%',
  ...
  sectionTitle: { fontSize: 17, fontWeight: '900', color: colors.text, letterSpacing: -0.25 },
  sectionText: { fontSize: 10, color: colors.muted, marginTop: 3, lineHeight: 15 },
  captureButton: {
    minHeight: 76,
    borderWidth: 1,
    borderColor: colors.primaryMist,
  ...
  captureTitle: { fontWeight: '900', color: colors.primaryDark },
  captureText: { color: colors.muted, marginTop: 3, fontSize: 10 },
  photoBox: {
    minHeight: 130,
    borderWidth: 1.5,
    borderStyle: 'dashed',
  ...
  dwellingPhoto: { width: '100%', height: 180 },
  citizenPhotoBox: {
    minHeight: 360,
    borderWidth: 1.5,
    borderStyle: 'dashed',
  ...
  citizenPhoto: { width: '100%', height: 430 },
  photoTitle: { marginTop: 10, fontSize: 16, fontWeight: '900', color: colors.text },
  personRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 11,
  ...
  personThumb: { width: 50, height: 50, borderRadius: 14 },
  personThumbEmpty: {
    width: 50,
    height: 50,
    borderRadius: 14,
  ...
  personName: { fontWeight: '900', color: colors.text },
  personMeta: { fontSize: 10, color: colors.muted, marginTop: 3 },
  addCitizen: {
    minHeight: 56,
    borderWidth: 1.5,
    borderStyle: 'dashed',
  ...
  choiceLabel: { fontSize: 12, fontWeight: '800', color: colors.textSoft },
  ...
  choiceActive: { borderColor: colors.primaryMist, backgroundColor: colors.primarySoft },
  ...
  ageStrong: { marginTop: 3, color: colors.text, fontWeight: '900' },
  error: {
  ...
  previousText: { fontWeight: '800', color: colors.textSoft },
  next: { minWidth: 180 },
  photoSourceGrid: { flexDirection: 'row', gap: 10 },
  photoSourceGridSmall: { flexDirection: 'column' },
  photoSourceButton: {
    flex: 1,
    minHeight: 76,
    borderWidth: 1,
  ...
  photoSourceCamera: { backgroundColor: colors.primarySoft, borderColor: colors.primaryMist },
  photoSourceTitle: { color: colors.text, fontWeight: '900', fontSize: 13 },
  photoSourceText: { color: colors.muted, fontSize: 10, marginTop: 3 },
  photoEmptyIcon: {
    width: 74,
    height: 74,
    borderRadius: 25,
  ...
  retakeFloating: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'rgba(15,39,71,.86)',
  },
  informationCard: {
    padding: 14,
    borderWidth: 1,
  ...
  reviewPhoto: { width: 60, height: 76, borderRadius: 15 },
  reviewName: { color: colors.text, fontWeight: '900', fontSize: 15 },
  reviewText: { color: colors.muted, fontSize: 10, marginTop: 4 },
})
