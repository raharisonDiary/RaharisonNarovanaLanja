              ...
              renderItem={({ item }) => {
                ...
                return (
                  <Pressable style={({ pressed }) => [styles.option, active && { backgroundColor: palette.primarySoft }, pressed && { backgroundColor: palette.surfaceSubtle }]} onPress={() => { onChange(item.value); close() }}>
                    <Text style={[styles.optionText, { color: active ? (palette.mode === 'dark' ? palette.primary : palette.primaryHover) : palette.text }, active && styles.optionTextActive]}>{item.label}</Text>
                    {active ? <View style={[styles.check, { backgroundColor: palette.primary }]}><Check color={palette.inverseText} size={14} /></View> : null}
                  </Pressable>
                )
              }}
            />
