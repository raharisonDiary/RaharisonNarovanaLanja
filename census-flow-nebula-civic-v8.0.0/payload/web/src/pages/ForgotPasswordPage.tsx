import { ArrowLeft, ArrowRight, CheckCircle2, Eye, EyeOff, KeyRound, Mail, MessageCircle, ShieldCheck } from 'lucide-react'
...
import type { PasswordRecoveryChannel } from '../types/api'
...
export default function ForgotPasswordPage() {
  ...
  const requestCode = async (event: FormEvent) => {
    ...
    setError('')
    try {
      const result = await passwordRecoveryApi.request(email.trim(), channel)
    ...
    <main className="recovery-page">
      ...
      <section className="recovery-card scale-in">
        ...
        {step === 'email' && <form onSubmit={requestCode} className="recovery-form">
          ...
          <h2>{t('emailQuestion')}</h2>
          <p>Saisissez l’adresse utilisée pour vous connecter, puis choisissez où recevoir le code OTP.</p>
          ...
          <div className="recovery-channel-grid" role="radiogroup" aria-label="Canal d’envoi du code OTP">
            <button type="button" className={`recovery-channel ${channel === 'Email' ? 'active' : ''}`} onClick={() => setChannel('Email')}><Mail size={21} /><strong>E-mail</strong><small>Envoyer le code à l’adresse du compte</small></button>
            <button type="button" className={`recovery-channel ${channel === 'WhatsApp' ? 'active' : ''}`} onClick={() => setChannel('WhatsApp')}><MessageCircle size={21} /><strong>WhatsApp</strong><small>Envoyer le code au numéro associé au compte</small></button>
          </div>
        ...
        {step === 'otp' && <form onSubmit={verifyCode} className="recovery-form">
          ...
          <h2>{t('enterSecurityCode')}</h2>
          <p>{message}</p>
          <div className="otp-delivery-note">{channel === 'WhatsApp' ? <MessageCircle size={17} /> : <Mail size={17} />} Le code n’est jamais affiché dans l’application.</div>
          ...
          <button className="link-button" type="button" onClick={() => setStep('email')}>Changer l’adresse ou le canal</button>
        </form>}
