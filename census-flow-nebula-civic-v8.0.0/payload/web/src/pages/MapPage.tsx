...
const hasAncestor = (leafId: string, ancestorId: string, areas: AdministrativeAreaDto[]) => {
  const byId = new Map(areas.map((area) => [area.id, area]))
  let current = byId.get(leafId)
...
export default function MapPage() {
  ...
  const countryArea = (areas.data ?? []).find((area) => area.type === 'Country' && area.name === country)
  const filteredCampaigns = useMemo(() => (campaigns.data ?? []).filter((item) => {
    const matchesText = item.name.toLocaleLowerCase().includes(campaignSearch.toLocaleLowerCase())
    const matchesCountry = !countryArea || hasAncestor(item.scopeAdministrativeAreaId, countryArea.id, areas.data ?? [])
    return matchesText && matchesCountry
  }), [areas.data, campaignSearch, campaigns.data, countryArea])
  const concernedRegions = useMemo(() => {
    if (!campaign) return []
    const scope = (areas.data ?? []).find((area) => area.id === campaign.scopeAdministrativeAreaId)
    if (!scope) return []
    if (scope.type === 'Country') return (areas.data ?? []).filter((area) => area.type === 'Region' && area.parentId === scope.id)
    const region = findRegion(scope.id, areas.data ?? [])
