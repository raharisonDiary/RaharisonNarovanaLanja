...
export default function PersonsPage() {
  ...
  const findPotentialDuplicate = () => {
    ...
    const normalizedNationalId = normalize(form.nationalId)
    const normalizedBirthPlace = normalize(form.birthPlace)

    return (persons.data ?? []).find((item) => {
      if (item.id === editing?.id) return false

      if (
        normalizedNationalId &&
        normalize(item.nationalId) === normalizedNationalId
      ) {
        return true
      }
      ...
      if (
        selectedHousehold &&
        item.campaignId !== selectedHousehold.campaignId
      ) {
        return false
      }

      if (
        normalize(item.firstName) !== normalize(form.firstName) ||
        normalize(item.lastName) !== normalize(form.lastName)
      ) {
        return false
      }

      if (form.birthMode === 'exact' && form.dateOfBirth) {
        return (
          item.dateOfBirth === form.dateOfBirth &&
          normalize(item.birthPlace) === normalizedBirthPlace
        )
      }

      if (form.birthMode === 'year' && form.birthYear) {
        return (
          item.dateOfBirth?.slice(0, 4) === form.birthYear &&
          normalize(item.birthPlace) === normalizedBirthPlace
        )
      }

      if (form.birthMode === 'age' && computedAge !== null) {
        return (
          item.ageYears === computedAge &&
          normalize(item.birthPlace) === normalizedBirthPlace
        )
      }

      return (
        item.householdId === form.householdId &&
        normalize(item.birthPlace) === normalizedBirthPlace
      )
    })
  }
  ...
  const save = async () => {
    ...
    if (duplicate) {
      setError(`${t('duplicateCitizenDetected')} : ${duplicate.fullName}.`)
      return
    }
    ...
    try {
      ...
      if (editing) {
        window.alert(t('citizenSavedSuccess'))
        setWizardOpen(false)
      } else if (
        window.confirm(
          `${t('citizenSavedSuccess')}\n\n${t('addAnotherCitizenQuestion')}`,
        )
      ) {
