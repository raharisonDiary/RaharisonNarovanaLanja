    ...
    : IPersonRepository
{
    ...
    public async Task<Person> AddWithGeneratedNumberAsync(
        Guid householdId,
        Func<int, Person> personFactory,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(personFactory);

        await using var transaction =
            await dbContext.Database.BeginTransactionAsync(
                cancellationToken);

        // Serialise uniquement les créations d'un même ménage.
        // Cela empêche deux appareils d'obtenir le même numéro.
        await dbContext.Database.ExecuteSqlRawAsync(
            "SELECT pg_advisory_xact_lock(hashtextextended({0}, 0))",
            [householdId.ToString()],
            cancellationToken);

        var highestNumber = await dbContext.Set<Person>()
            .IgnoreQueryFilters()
            .Where(person => person.HouseholdId == householdId)
            .Select(person => (int?)person.PersonNumber)
            .MaxAsync(cancellationToken);

        var nextNumber = checked((highestNumber ?? 0) + 1);
        var person = personFactory(nextNumber);
