...
const cleanBaseUrl = (value: string): string =>
  value.replace(/\/+$/, '')

const webBaseURL = cleanBaseUrl(
  process.env.EXPO_PUBLIC_API_URL_WEB ??
    'http://localhost:5001/api/v1',
)

const nativeBaseURL = cleanBaseUrl(
  process.env.EXPO_PUBLIC_API_URL_NATIVE ??
    process.env.EXPO_PUBLIC_API_URL ??
    'http://192.168.1.10:5001/api/v1',
)

const baseURL =
  Platform.OS === 'web' ? webBaseURL : nativeBaseURL
