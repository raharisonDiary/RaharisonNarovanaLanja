...
export default function PersonsScreen() {
  ...
  const findPotentialDuplicate = () => {
    ...
    const normalizedFirstName = normalize(form.firstName)
    const normalizedLastName = normalize(form.lastName)
    const normalizedBirthPlace = normalize(form.birthPlace)
    const normalizedNationalId = normalize(form.nationalId)
    const selectedCampaignId = selectedHousehold?.campaignId
    ...
    return items.find((item) => {
      ...
      if (
        normalize(item.firstName) !== normalizedFirstName ||
        normalize(item.lastName) !== normalizedLastName
      ) {
        return false
      }
