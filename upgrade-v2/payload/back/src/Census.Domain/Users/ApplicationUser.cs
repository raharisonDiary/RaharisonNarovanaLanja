...
public sealed class ApplicationUser : EntityBase
{
    ...
    // Une seule récupération active par utilisateur. Les secrets ne sont jamais stockés en clair.
        ...
        int maximumLength)
    {
        ...
        {
            throw new ArgumentException($"La valeur ne peut pas dépasser {maximumLength} caractères.", parameterName);
        }
    ...
    private static string? NormalizeOptionalValue(string? value, int maximumLength)
    {
        ...
        if (normalizedValue.Length > maximumLength)
        {
            throw new ArgumentException($"La valeur ne peut pas dépasser {maximumLength} caractères.");
        }
        return normalizedValue;
    }
}
