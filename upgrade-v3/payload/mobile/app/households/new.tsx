...
import { ArrowLeft, Building2, Camera, Check, ChevronLeft, ChevronRight, LocateFixed, Plus, Save, Trash2, UserRound, UsersRound } from 'lucide-react-native'
...
export default function NewHouseholdScreen() {
    ...
    void Promise.all([mobileApi.campaigns(), mobileApi.areas()]).then(async ([campaignRows, areaRows]) => {
      ...
      if (editId) {
        ...
        if (existing) {
          ...
          setLatitude(String(bundle.location.latitude)); setLongitude(String(bundle.location.longitude)); setAccuracy(bundle.location.accuracy ?? null); setAddress(bundle.dwelling.address); setDwellingPhoto(bundle.dwelling.photoUri ?? null)
          setHouseholdCode(bundle.household.referenceCode); setHouseholdType(bundle.household.householdType); setPhone(bundle.household.phoneNumber); setPersons(bundle.persons); setCitizen(emptyCitizen(bundle.persons.length + 1))
        }
      }
  ...
  const takePhoto = async (forCitizen = false) => {
    ...
    const result = await ImagePicker.launchCameraAsync({ quality: .65, allowsEditing: true, aspect: [3, 4] })
    if (!result.canceled) { if (forCitizen) setCitizen((value) => ({ ...value, photoUri: result.assets[0].uri })); else setDwellingPhoto(result.assets[0].uri) }
  }
  ...
  const addCitizen = () => {
    ...
    const normalized = { ...citizen, ageYears: citizen.birthDateMode === 'age' ? citizen.ageYears : age, nationalId: adult && !citizen.hasNoNationalId ? citizen.nationalId : '', nationalIdIssueDate: adult && !citizen.hasNoNationalId ? citizen.nationalIdIssueDate : '', nationalIdIssuePlace: adult && !citizen.hasNoNationalId ? citizen.nationalIdIssuePlace : '', childrenCount: ['Married', 'Widowed'].includes(citizen.maritalStatus) ? citizen.childrenCount : null }
  ...
  const saveLocal = async () => {
    if (!campaignId || !fokontanyId || !latitude || !longitude || persons.length === 0) { setError(t('completeTerritory')); return }
    ...
    const bundle: CensusBundle = {
      ...
      dwelling: { referenceCode: randomCode('HAB'), address, localityName: areaName(fokontanyId), latitude: Number(latitude), longitude: Number(longitude), photoUri: dwellingPhoto },
      household: { referenceCode: householdCode, householdType, headFullName: `${persons[0].firstName} ${persons[0].lastName}`.trim(), phoneNumber: phone }, persons,
      createdAt: editId ? (await getQueueItem(editId))?.payload.createdAt ?? now : now, updatedAt: now,
    }
    try { await enqueueBundle(bundle); router.replace({ pathname: '/confirmation', params: { code: householdCode, queued: 'true' } }) }
  ...
  const canContinue = () => {
    ...
    if (step === 1) return Boolean(householdCode)
    if (step === 2) return persons.length > 0
  ...
  return <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.card}>
        ...
        {step === 1 && <><Section icon={Building2} title={`2. ${t('householdInfo')}`} text={t('autoCode')} /><FormField label={t('householdCode')} value={householdCode} onChangeText={setHouseholdCode} /><SelectField label={t('householdType')} value={householdType} onChange={setHouseholdType} options={[{ label: t('ordinaryHousehold'), value: 'Ordinary' }, { label: t('collectiveHousehold'), value: 'Collective' }, { label: t('homeless'), value: 'Homeless' }]} /><FormField label={t('householdPhone')} value={phone} onChangeText={setPhone} keyboardType="phone-pad" placeholder={t('optional')} /></>}
        {step === 2 && <><Section icon={UsersRound} title={`3. ${t('citizensHousehold')}`} text={t('citizensDraftText')} />{persons.map((person, index) => <View key={person.localId} style={styles.personRow}>{person.photoUri ? <Image source={{ uri: person.photoUri }} style={styles.personThumb} /> : <View style={styles.personThumbEmpty}><UserRound color={colors.muted} size={22} /></View>}<View style={{ flex: 1 }}><Text style={styles.personName}>{index + 1}. {person.firstName} {person.lastName}</Text><Text style={styles.personMeta}>{person.ageYears ?? t('unknownAge')} {t('years')} · {person.relationshipToHead}</Text></View><Pressable onPress={() => removeCitizen(person.localId)}><Trash2 color={colors.danger} size={20} /></Pressable></View>)}<Pressable style={styles.addCitizen} onPress={() => { setCitizen(emptyCitizen(persons.length + 1)); setStep(3) }}><Plus color={colors.primary} size={20} /><Text>{t('addCitizen')}</Text></Pressable>{persons.length > 0 && <PrimaryButton title={t('saveLocally')} icon={Save} loading={saving} onPress={() => void saveLocal()} />}</>}
        {step === 3 && <><Section icon={Camera} title={t('citizenPhoto')} text={t('citizenPhotoText')} /><Pressable style={styles.citizenPhotoBox} onPress={() => void takePhoto(true)}>{citizen.photoUri ? <Image source={{ uri: citizen.photoUri }} style={styles.citizenPhoto} /> : <><Camera color={colors.primary} size={42} /><Text style={styles.photoTitle}>{t('takePhoto')}</Text><Text style={styles.captureText}>{t('faceCentered')}</Text></>}</Pressable></>}
