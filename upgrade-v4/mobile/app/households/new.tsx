...
export default function NewHouseholdScreen() {
  ...
  const takePhoto = async (forCitizen = false) => {
    ...
    const result = await ImagePicker.launchCameraAsync({ quality: .6, allowsEditing: true, aspect: [3, 4], base64: true })
    if (!result.canceled) {
      const asset = result.assets[0]
      const photoDataUrl = asset.base64 ? `data:${asset.mimeType ?? 'image/jpeg'};base64,${asset.base64}` : null
      if (forCitizen) setCitizen((value) => ({ ...value, photoUri: asset.uri, photoDataUrl })); else setDwellingPhoto(asset.uri)
    }
  }
