...
export default function PersonDetailPage() {
  ...
  const rows = [
    ...
    [t('birthDate'), formatDate(item.dateOfBirth)], [t('birthPlace'), item.birthPlace || '—'], [t('age'), item.ageYears ?? '—'],
    ...
    [t('phone'), item.phoneNumber || '—'], [t('nationalId'), item.nationalId || '—'], [t('issueDate'), formatDate(item.nationalIdIssueDate)], [t('issuePlace'), item.nationalIdIssuePlace || '—'], [t('childrenCount'), item.childrenCount ?? '—'],
  ...
  return <>
    ...
    <section className="person-detail">
      <aside className="card person-card">{item.photoDataUrl ? <img className="person-card__avatar" src={item.photoDataUrl} alt={item.fullName} /> : <span className="person-card__avatar"><UserRound size={60} /></span>}<QRCodeSVG value={`census://persons/${item.id}`} size={150} level="M" /><button className="button button--primary button--wide"><QrCode size={17} />{t('citizenQr')}</button></aside>
